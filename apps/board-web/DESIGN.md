...
Issue #77 overlays the round-four metro directly on the measured city. The line is projected from the published station contract into each live tile viewport, joins active squads in ordinal order, and labels a squad `Fora da rede` when its station or live tile is unavailable. The round-five register names each squad's next-neighbour dependency at both challenge and dispatch, while round six adds a recoverable model handoff record rather than a transient picker. `/finale` keeps the city first, then reads as a closing field sheet: aggregate draft and return totals precede one compact decision record per squad. Metro updates are immediate data changes; no additional animation competes with the map.

## Finish evidence

The base visual proof is in `.impeccable/review/desktop.png` and `.impeccable/review/mobile.png`. Issue #75 responsive evidence lives in `.impeccable/review/issue-75/`. Issue #76 proof is in `.impeccable/review/hero-repro.png` and `.impeccable/review/issue-76/mobile-city-tabs.png`, with admin, member, and steerer states beside them under `.impeccable/review/issue-76/`. Issue #77 desktop city and finale evidence plus their mobile counterparts live in `.impeccable/review/issue-77/`. The 1600×1000 desktop capture follows the approved map topology; mobile proof keeps the city first, preserves broken and empty lots, and shows the fixed route bar above the tile content.
