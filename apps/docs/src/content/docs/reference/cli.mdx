---
title: CLI Reference
description: Reference for the Gambi operational CLI.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

The Gambi CLI is the operational surface for hubs, rooms, participants, and room events.

The CLI is resource-oriented:

```bash
gambi hub serve
gambi room create
gambi room list
gambi room get
gambi participant join
gambi join # short alias for participant join
gambi participant leave
gambi participant heartbeat
gambi events watch
gambi self update
```

`gambi-tui` remains the separate human-first monitoring interface.

## Installation

<Tabs>
  <TabItem label="Linux / macOS" icon="linux">
    ```bash
    curl -fsSL https://raw.githubusercontent.com/arthurbm/gambi/main/scripts/install.sh | bash
    ```
  </TabItem>
  <TabItem label="Windows" icon="laptop">
    ```powershell
    irm https://raw.githubusercontent.com/arthurbm/gambi/main/scripts/install.ps1 | iex
    ```
  </TabItem>
  <TabItem label="npm / bun" icon="seti:npm">
    ```bash
    npm install -g gambi
    # or
    bun add -g gambi
    ```
  </TabItem>
</Tabs>

Verify the install:

```bash
gambi --version
```

## Global behavior

Every operational command inherits the same set of global flags from the shared command base. They can also be driven by environment variables, which is useful for scripts and supervisors.

### Global flags

| Flag | Description |
| --- | --- |
| `--format <text\|json\|ndjson>` | select output format |
| `--env <name>` | pick a named environment from the CLI config file |
| `--interactive` | allow prompts for missing required values |
| `--no-interactive` | disable all prompts |
| `--verbose` | include extended details in output |
| `--quiet` | reduce non-essential text output |

### Output modes

| Mode | Meaning |
| --- | --- |
| `text` | compact human-readable output |
| `json` | one structured JSON object |
| `ndjson` | one JSON object per line |

Defaults:

| Context | Default |
| --- | --- |
| TTY + one-shot command | `text` |
| piped stdout + one-shot command | `json` |
| piped stdout + streaming command | `ndjson` |

Streaming commands coerce `--format json` to `ndjson` so every event is a standalone JSON line.

### Environment variables

| Variable | Effect |
| --- | --- |
| `GAMBI_FORMAT` | fallback for `--format` |
| `GAMBI_ENV` | fallback for `--env` |
| `GAMBI_NO_INTERACTIVE=1` | disable prompts everywhere |
| `XDG_CONFIG_HOME` | override the base directory for `~/.config/gambi/config.json` |

### Exit codes

| Code | Meaning |
| --- | --- |
| `0` | success |
| `1` | internal or unexpected failure |
| `2` | invalid usage (missing flag, bad value, `400`/`422` from hub) |
| `3` | connectivity or dependency failure (unreachable hub, `401`/`403`/`503`) |
| `4` | remote rejection or conflict (`404`/`409` from hub) |

Agent-style supervisors can key retries off these codes. `2` should not be retried, `3` usually should, and `4` means the operation is rejected by the hub state.

### Named environments

The CLI reads XDG config from:

```text
~/.config/gambi/config.json
```

Example:

```json
{
  "defaultEnv": "local",
  "envs": {
    "local": {
      "hubUrl": "http://localhost:3000",
      "endpoint": "http://localhost:11434"
    },
    "lab": {
      "hubUrl": "http://192.168.1.10:3000",
      "endpoint": "http://localhost:11434",
      "serve": {
        "host": "0.0.0.0",
        "port": 3000,
        "mdns": true
      }
    }
  }
}
```

Select an environment per-invocation with `--env lab` or globally with `GAMBI_ENV=lab`.

## `gambi hub serve`

Start the Gambi hub.

```bash
gambi hub serve [options]
```

Options:

| Option | Description |
| --- | --- |
| `--host` | host to bind to (default from env config or `0.0.0.0`) |
| `--port` | port to bind to (default from env config or `3000`) |
| `--mdns` | enable mDNS discovery so other machines can find the hub |
| `--dry-run` | print the resolved startup plan and exit without binding |

Examples:

```bash
gambi hub serve
gambi hub serve --port 3000 --mdns
gambi hub serve --dry-run --format ndjson
```

Structured lifecycle events (`--format json|ndjson`):

| Event | Meaning |
| --- | --- |
| `started` | hub bound to `bindUrl` |
| `mdns_registered` | mDNS advertisement is active under `name` |
| `signal_received` | `SIGINT` or `SIGTERM` observed during shutdown |
| `stopped` | hub has released the port |

## `gambi room create`

Create a room through the management API.

```bash
gambi room create --name <name> [options]
```

Options:

| Option | Description |
| --- | --- |
| `--name`, `-n` | room display name |
| `--password`, `-p` | optional room password |
| `--config <path\|->` | runtime config JSON file or `-` for stdin |
| `--hub`, `-H` | hub URL |
| `--dry-run` | validate and print the request without creating the room |

Examples:

```bash
gambi room create --name "Demo"

gambi room create --name "Demo" --config ./room.json

cat ./room.json | gambi room create --name "Demo" --config -

gambi room create --name "Demo" --dry-run --format json
```

## `gambi room list`

List room summaries from the management API.

```bash
gambi room list [options]
```

Options:

| Option | Description |
| --- | --- |
| `--hub`, `-H` | hub URL |
| `--sort <participant-count\|created-at\|name>` | sort key (default `participant-count`) |
| `--reverse` | reverse the selected sort order |

Default sort is `participant-count` descending, with `created-at` descending as tiebreaker.

Examples:

```bash
gambi room list
gambi room list --format json
gambi room list --sort created-at --reverse
gambi room list --hub http://192.168.1.10:3000
```

## `gambi room get`

Fetch one room summary by code.

```bash
gambi room get --code <room-code> [options]
```

Options:

| Option | Description |
| --- | --- |
| `--code`, `-c` | room code |
| `--hub`, `-H` | hub URL |

Examples:

```bash
gambi room get --code ABC123
gambi room get --code ABC123 --format json
```

## `gambi participant join`

Register a model endpoint or local ACP harness and keep its tunnel alive.

```bash
gambi participant join --room <room> --participant-id <id> --model <model> [options]
gambi join --room <room> --participant-id <id> --harness <id> [options]
```

Options:

| Option | Description |
| --- | --- |
| `--room` | room code |
| `--participant-id` | stable participant identifier |
| `--nickname`, `--name` | display name (`--name` is an alias) |
| `--model` | model to expose, or an optional label in harness mode |
| `--harness <opencode\|claude-code\|codex\|fake>` | start a local ACP harness instead of a model endpoint |
| `--endpoint` | local endpoint used for probing and inference |
| `--header` | auth header in `Header=Value` format |
| `--header-env` | auth header in `Header=ENV_VAR` format |
| `--password` | room password |
| `--config <path\|->` | runtime config JSON file or stdin |
| `--no-specs` | skip machine specs collection |
| `--hub` | hub URL |
| `--dry-run` | validate and preview the session |

Notes:

- `--participant-id` is required for non-interactive retry-safe automation.
- `--config -` reads participant runtime config JSON from stdin.
- The CLI probes the endpoint before registration to validate the model and detect capabilities.
- Participant joins now always use a tunnel, so the provider endpoint can stay on `localhost` even when the hub runs on another machine. See [How tunnels work](/explanation/tunnel-first/).
- Harness mode creates `~/.gambi/workspaces/<room>/<participant-id>/`, adds the city tile starter when files are missing, starts the ACP agent in that directory, and publishes changed workspace files after a one-second debounce.
- `--endpoint`, `--header`, and `--header-env` are invalid with `--harness`. The harness uses its existing login on the participant machine. Gambi does not read or upload those credentials.
- The `opencode` adapter runs `opencode acp`. If OpenCode is missing or has no detected credentials, the command exits with code `3` and prints the command needed to fix it.
- The `claude-code` adapter runs the published `claude-agent-acp` bridge. Install it with `npm install -g @agentclientprotocol/claude-agent-acp`, install Claude Code with `npm install -g @anthropic-ai/claude-code`, and authenticate locally with `claude auth login`.
- **Claude Code terms warning:** each end user must run the unmodified Anthropic binary with their own local authentication. Gambi never intermediates a Claude subscription login and must never host Claude Code for third parties. Review Anthropic's [legal and compliance terms](https://code.claude.com/docs/en/legal-and-compliance) before joining.
- The `codex` adapter runs the published `codex-acp` bridge. Install it with `npm install -g @agentclientprotocol/codex-acp`, install Codex CLI with `npm install -g @openai/codex`, and authenticate locally with `codex login`.
- Claude Code and Codex readiness checks use `claude auth status --json` and `codex login status`. Authentication state and credentials remain on the participant machine and are never included in registration, lifecycle events, or tunnel frames.
- The `fake` adapter is deterministic and calls no model. It is for the event rehearsal and automated tests.
- `--dry-run` validates the command input and prints the registration preview. It does not start the ACP process or verify the harness login.

For a task-oriented setup for OpenCode, Claude Code, and Codex, see [Bring your harness](/guides/bring-your-harness/).

Examples:

```bash
gambi participant join \
  --room ABC123 \
  --participant-id worker-1 \
  --model llama3

gambi participant join \
  --room ABC123 \
  --participant-id worker-1 \
  --model llama3 \
  --endpoint http://localhost:1234

gambi participant join \
  --room ABC123 \
  --participant-id worker-1 \
  --model llama3 \
  --hub http://192.168.1.10:3000

gambi participant join \
  --room ABC123 \
  --participant-id worker-1 \
  --model llama3 \
  --dry-run \
  --format ndjson

gambi join \
  --room ABC123 \
  --participant-id arthur-opencode \
  --name Arthur \
  --harness opencode

gambi join \
  --room ABC123 \
  --participant-id bia-claude \
  --name Bia \
  --harness claude-code

gambi join \
  --room ABC123 \
  --participant-id caio-codex \
  --name Caio \
  --harness codex

gambi join \
  --room ABC123 \
  --participant-id rehearsal-1 \
  --harness fake \
  --model deterministic-fixture
```

NDJSON lifecycle events:

| Event | Meaning |
| --- | --- |
| `prepared` | endpoint probing and payload assembly completed |
| `registered` | participant registration succeeded |
| `tunnel_connected` | the participant tunnel is open |
| `harness_spawned` | the local ACP process started |
| `session_opened` | ACP v1 negotiation and `session/new` succeeded |
| `artifact_sent` | a versioned workspace snapshot crossed the tunnel |
| `harness_exited` | the local ACP process exited |
| `leaving` | shutdown started |
| `left` | participant removal completed |
| `heartbeat_failed` | management heartbeat loop failed |
| `tunnel_failed` | tunnel connectivity failed |

## `gambi participant leave`

Remove a participant from a room.

```bash
gambi participant leave --room <room> --participant-id <id> [options]
```

Options:

| Option | Description |
| --- | --- |
| `--room`, `-r` | room code |
| `--participant-id` | participant identifier |
| `--hub`, `-H` | hub URL |

Example:

```bash
gambi participant leave --room ABC123 --participant-id worker-1
```

## `gambi participant heartbeat`

Send one participant heartbeat. Useful for automation and liveness probing independently from the long-running `participant join` command.

```bash
gambi participant heartbeat --room <room> --participant-id <id> [options]
```

Options:

| Option | Description |
| --- | --- |
| `--room`, `-r` | room code |
| `--participant-id` | participant identifier |
| `--hub`, `-H` | hub URL |

Example:

```bash
gambi participant heartbeat --room ABC123 --participant-id worker-1
```

## `gambi events watch`

Watch room events from the management SSE stream.

```bash
gambi events watch --room <room> [options]
```

Options:

| Option | Description |
| --- | --- |
| `--room`, `-r` | room code (required) |
| `--hub`, `-H` | hub URL |

Examples:

```bash
gambi events watch --room ABC123
gambi events watch --room ABC123 --format ndjson
```

Important event types:

- `connected`
- `room.created`
- `participant.joined`
- `participant.updated`
- `participant.left`
- `participant.offline`
- `llm.request`
- `llm.complete`
- `llm.error`

See [Observability](/reference/observability/) for the field shape of the `llm.*` events.

## `gambi self update`

Update the installed CLI. The command detects the install source (bun, npm, or standalone binary) and runs the matching upgrade path.

```bash
gambi self update [options]
```

Options:

| Option | Description |
| --- | --- |
| `--manager <bun\|npm>` | force a package manager instead of auto-detection |
| `--yes` | skip confirmation prompt |
| `--dry-run` | print the resolved update plan and exit |

Examples:

```bash
gambi self update
gambi self update --yes
gambi self update --dry-run --format json
```

## See also

- [API Reference](/reference/api/) for the HTTP surfaces backing the CLI.
- [SDK Reference](/reference/sdk/) for programmatic equivalents.
- [How tunnels work](/explanation/tunnel-first/) for the conceptual model behind `participant join`.
