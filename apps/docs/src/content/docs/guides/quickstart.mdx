...
Important implication: your provider endpoint can stay on `localhost`, even when the hub is running on another machine in the same trusted network. You no longer need to publish a LAN-reachable provider URL just to join a room. For the reasoning behind this, see [How Tunnels Work](/explanation/tunnel-first/).
...
- Building multi-LLM experiences? See [Multi-LLM Patterns](/guides/patterns/)
