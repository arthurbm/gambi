...
For local Node.js or Bun applications, you can also discover the hub and room first, then create the provider explicitly:
...
This discovery flow is useful for local apps generated by tools like Claude Code or Cursor that run on your machine. Hosted app builders such as Lovable or v0 still need an explicit OpenAI-compatible base URL because they do not execute your local SDK directly.
