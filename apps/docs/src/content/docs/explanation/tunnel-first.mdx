...
Harness participants use the same direction of trust. Their ACP agent stays local and sends opaque messages and artifacts through the participant-opened tunnel. A board or orchestrator opens its own management WebSocket to the hub and attaches to that participant; the hub only relays between connections it did not originate. Harness participants are not candidates for inference routing.
