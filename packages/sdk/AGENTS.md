...
- `src/participant-session.ts` — reexport de `createParticipantSession()` e `ParticipantSession` implementados em `@gambi/core/participant-session` (close reasons: `"closed"`, `"heartbeat_failed"`, `"tunnel_closed"`).
...
- **`createParticipantSession()` é o runtime canônico**: a implementação vive em `@gambi/core/participant-session`, e a SDK reexporta essa função para usuários externos. Novos runtimes custom devem usar essa mesma função em vez de reimplementar tunelamento.
